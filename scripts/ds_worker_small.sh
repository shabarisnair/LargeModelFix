#!/usr/bin/env bash
# DeepSeek baselines -- WORKER A (uses ONLY the small model, :8000).
#
...
#
# Fairness: this worker is the ONLY user of the small model while it runs, and it
# never touches the large model -- so it can run concurrently with worker B's
# `large` baseline without either contending for a server.
set -euo pipefail
